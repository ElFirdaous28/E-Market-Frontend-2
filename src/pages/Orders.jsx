                ...
                <div className="space-y-4">
                    ...
                    {!isLoading && userOrders?.length === 0 && (
                        <div className="text-center py-16">
                            ...
                            <a href="/" className="inline-block px-6 py-3 bg-primary text-textMain rounded-lg hover:bg-opacity-90 transition font-semibold">
                                Start Shopping
                            </a>
                        </div>
                    )}
                    ...
                    {userOrders?.map((order) => (
                        <div key={order._id} className="bg-background rounded-lg max-w-4xl w-full max-h-[90vh] border border-border overflow-y-auto hide-scrollbar">
                            {/* Order Header */}
                            <div className="bg-background px-6 py-4 border-b border-border">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-primary">${order.finalAmount.toFixed(2)}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-textMuted mb-1">STATUS</p>
                                        <span className={`text-sm font-medium inline-block py-1 rounded-full ${statusColors[order.status]}`}>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-background rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto hide-scrollbar">
                        ...
                        <div className="sticky top-0 bg-background border-b border-border px-6 py-4 flex justify-between items-center">
                            ...
                            <button onClick={closeModal} className="text-textMuted hover:text-textMain-light dark:hover:text-textMain-dark">
                            ...
                            <div className="bg-surface rounded-lg p-6 mb-6">
                                ...
                                <div className="space-y-4">
                                    {selectedOrder?.items?.map((item) => (
                                        <div key={item._id} className="bg-background rounded-lg flex items-center gap-4 p-4 border-b border-border">
                                            ...
                                            <div className="flex-1">
                                                <h4 className="font-semibold text-textMain mb-1">{item?.productId?.title}</h4>
                                                <p className="text-sm text-textMuted mb-2">{item?.productId?.description}</p>
                                ...
                                <div className="bg-surface border border-border rounded-lg p-6">
                                    ...
                                    <div className="space-y-2 mb-4">
                                        ...
                                        <div className="flex justify-between text-success-light dark:text-success-dark">
                                            <span>Discount</span>
                                            <span>${(selectedOrder?.totalAmount - selectedOrder?.finalAmount).toFixed(2)}</span>
                                        </div>
