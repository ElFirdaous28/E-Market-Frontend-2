                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-800">
                            <tr className="text-left">
                                ...
                                <th className="px-4 py-2 font-medium text-gray-700 dark:text-gray-200">Total (vous)</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                            {orders.map(order => {
                                const productTitles = order.items?.map(it => it.productId?.title).filter(Boolean) || [];
                                const productsDisplay = productTitles.slice(0, 3).join(', ');
                                const moreCount = productTitles.length - 3;
                                const dateStr = new Date(order.createdAt).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' });
                                return (
                                    <tr key={order._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                        ...
                                        <td className="px-4 py-2 max-w-xs">
                                            <div title={productTitles.join(', ')} className="truncate">
                                                {productsDisplay || '—'}{moreCount > 0 && ` …(+${moreCount})`}
                                            </div>
                                        </td>
                                        <td className="px-4 py-2">
                                            {typeof order.sellerTotal === 'number' ? order.sellerTotal.toFixed(2) : '0.00'} €
                                        </td>
                                        <td className="px-4 py-2">
                                        ...
                                        <td className="px-4 py-2">
                                            <select
                                                ...
                                                value={order.status}
                                                disabled={statusUpdating || order.status === 'delivered' || order.status === 'cancelled'}
