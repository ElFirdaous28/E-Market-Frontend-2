...
export default function ProductsManager() {
    ...
    const loading = (showDeleted ? deletedProductsQuery.isLoading : productsQuery.isLoading) || deleteMutation.isPending;
                ...
                {!loading && filtered.length === 0 && (
                    <div className="p-8 text-center">
                        ...
                        <p className="text-textMuted">
                            {search ? 'Aucun produit trouvé.' : showDeleted ? 'Aucun produit supprimé.' : 'Aucun produit. Créez votre premier produit.'}
                        </p>
                    </div>
                )}
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-surface/50 border-b border-border">
                                <tr>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Produit</th>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Prix</th>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Stock</th>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Statut</th>
                                    <th className="text-right px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border">
                                {paginated.map(p => (
                                    <tr key={p._id} className="hover:bg-primary/5 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 bg-surface border border-border rounded-lg overflow-hidden shrink-0">
                                                    {p.primaryImage ? (
                                                        <img src={`${import.meta.env.VITE_API_URL}` + p.primaryImage} alt={p.title} className="w-full h-full object-cover" />
                                                    ) : (
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    {p.description && (
                                                        <div className="text-sm text-textMuted truncate max-w-md">{p.description}</div>
                                                    )}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${p.stock > 10 ? 'bg-green-100 text-green-800' :
                    ...
                    <div className="px-6 py-4 border-t border-border bg-surface/50">
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-textMuted">
                                Affichage de {((currentPage - 1) * pageSize) + 1} à {Math.min(currentPage * pageSize, filtered.length)} sur {filtered.length} produits
                            </div>
