    ...
    return (
        <div className="space-y-6 max-w-6xl w-full mx-auto">
            ...
            {(productsQuery.isLoading || ordersQuery.isLoading) && (
                <div className="bg-surface border border-border rounded-lg p-4 text-textMuted">Chargement des statistiques...</div>
            )}
            {ordersQuery.isError && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard icon={DollarSign} label="Revenu total" value={`${stats.totalRevenue.toFixed(2)} DH`} color="text-emerald-600" bg="bg-emerald-50" />
                <StatCard icon={Clock} label="Commandes en attente" value={stats.pendingOrders} color="text-amber-600" bg="bg-amber-50" />
                <StatCard icon={CheckCircle2} label="Commandes livrées" value={stats.deliveredOrders} color="text-indigo-600" bg="bg-indigo-50" />
                <StatCard icon={Package} label="Total produits" value={stats.totalProducts} color="text-primary" bg="bg-primary/10" />
            </div>
            ...
            <div className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden">
                ...
                {recentOrders.length === 0 ? (
                    <div className="p-8 text-center text-textMuted">
                        <div className="flex items-center justify-center mb-2"><Receipt className="w-6 h-6 text-textMuted/40" /></div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-surface/50 border-b border-border">
                                <tr>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">#</th>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Statut</th>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Total</th>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-textMuted uppercase tracking-wider">Date</th>
                                </tr>
                            </thead>
