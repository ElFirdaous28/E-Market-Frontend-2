        ...
        <div className="w-6xl mx-auto space-y-6 p-4 sm:p-6">
                    ...
                    <table className="min-w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-800">
                            <tr className="text-left">
                                <th className="px-6 py-3.5 font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider text-xs">Code</th>
                                <th className="px-6 py-3.5 font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider text-xs">Type</th>
                                <th className="px-6 py-3.5 font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider text-xs">Valeur</th>
                                <th className="px-6 py-3.5 font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider text-xs">Usage max</th>
                                <th className="px-6 py-3.5 font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider text-xs">Statut</th>
                                <th className="px-6 py-3.5 font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider text-xs">Validité</th>
                                <th className="px-6 py-3.5 font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider text-xs">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
                            {loading && (
                                <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">Chargement...</td></tr>
                            )}
                            {!loading && coupons.length === 0 && (
                                <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">Aucun coupon trouvé.</td></tr>
                            )}
                            {!loading && coupons.map(c => {
                                ...
                                return (
                                    <tr key={c._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/70 transition-colors">
                                        <td className="px-6 py-4 font-semibold text-gray-900 dark:text-gray-100">{c.code}</td>
                                        <td className="px-6 py-4 capitalize text-gray-700 dark:text-gray-300">{c.type}</td>
                                        <td className="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{c.value}{c.type === 'percentage' ? '%' : '€'}</td>
                                        <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{c.maxUsage ?? '—'}</td>
                                        <td className="px-6 py-4">
                                            <span className={c.status === 'active' ? 'px-2.5 py-1 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs font-medium' : 'px-2.5 py-1 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs font-medium'}>
                                                {c.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-400 text-xs" title={`${start} → ${end}`}>{start} → {end}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex gap-3">
                                                <Link to={`/seller/coupons/edit/${c._id}`} className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium transition-colors">Éditer</Link>
                                                <button
            ...
            {totalPages > 1 && (
                <div className="flex gap-3 justify-end items-center flex-wrap">
                    <span className="text-sm text-gray-600 dark:text-gray-400 font-medium">Page {page} / {totalPages}</span>
                    <button
            ...
            {confirmId && (
                <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-md shadow-2xl space-y-5 border border-gray-200 dark:border-gray-700">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">Confirmer suppression</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">Êtes-vous sûr de vouloir supprimer ce coupon ? Cette action est irréversible.</p>
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <button
                                ...
                                disabled={deleteMutation.isPending}
                            >{deleteMutation.isPending ? 'Suppression...' : 'Supprimer'}</button>
                        </div>
                    </div>
                </div>
            )}
