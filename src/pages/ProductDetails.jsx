  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
          ...
          <div className="bg-surface rounded-2xl border border-border flex items-center justify-center aspect-square relative overflow-hidden group shadow-sm">
            ...
            {allImages.length > 1 && (
              <>
                <button
                  onClick={prevImage}
                  className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/90 shadow-lg hover:bg-white text-textMain transition-all opacity-0 group-hover:opacity-100 -translate-x-2.5 group-hover:translate-x-0"
                >
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button
                  onClick={nextImage}
                  className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/90 shadow-lg hover:bg-white text-textMain transition-all opacity-0 group-hover:opacity-100 translate-x-2.5 group-hover:translate-x-0"
                >
      ...
      <div className="border-t border-border pt-16">
        <div className="flex flex-col max-w-4xl mx-auto">
          <div className="flex justify-between font-bold text-textMain mb-8">
            ...
            <button
              className="py-2 px-4 border border-primary rounded-lg hover:bg-primary"
          ...
          <div className="flex flex-wrap justify-between items-center gap-4 mb-8 bg-surface p-4 rounded-lg border border-border/50">
            <div className="flex items-center gap-3">
              <span className="text-4xl font-bold text-textMain">
                {productReviews?.averageRating.toFixed(2) || 0}
              </span>
          ...
          <div className="space-y-6">
            {productReviews?.reviews?.map((review) => (
              <div
                key={review._id}
                className="bg-surface rounded-2xl border border-border p-8 hover:shadow-md transition-shadow duration-300"
              >
                <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        <div className="flex text-yellow-400">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`w-3 h-3 ${i < Math.round(review.rating || 0) ? 'fill-yellow-400' : 'text-gray-300'}`}
                            />
                          ))}
                        </div>
                  ...
                  {review.user._id === user._id && (
                    <div className="flex gap-2">
                      <button
                        className="p-2 text-textMuted hover:text-primary hover:bg-primary/10 rounded-lg transition"
                ...
                <p className="text-textMuted leading-relaxed mb-6">{review.comment}</p>
      ...
      {isModalOpen && (
        <div
          id="reviewModal"
          className=" fixed inset-0 bg-black/75 bg-opacity-50 z-50 flex items-center justify-center"
        >
          <div className="bg-surface rounded-lg shadow-xl w-full max-w-lg mx-4 overflow-hidden">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center">
              <h3 id="modalTitle" className="text-xl font-bold text-textMain">
                Write a Review
              </h3>
              <button
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-textMain mb-2">Rating</label>
              ...
              {serverError && <p className="text-red-500 text-sm mt-2 ml-2">{serverError}</p>}
            </div>

            <div className="px-6 py-4 border-t border-border flex justify-end space-x-3 bg-surface">
              <button
                onClick={() => setIsModalOpen(false)}
