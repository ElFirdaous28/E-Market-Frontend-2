...
describe('AdminActivities Component', () => {
  test('renders the header text', () => {
    ...
    expect(
      screen.getByText('Track all the latest actions and updates in your store')
    ).toBeInTheDocument();
  });
  ...
  test('renders activities from mocked hook', () => {
    ...
    // Activity details
    expect(screen.getByText('Admin John created a product')).toBeInTheDocument();

    expect(screen.getByText('Seller Jane updated a product')).toBeInTheDocument();
