...
describe('Cart Integration Tests', () => {
  ...
  it('Given a product in the cart, When the quantity is changed, Then the total is recalculated', async () => {
    const mockCartData = {
      items: [{ _id: 'item1', productId: { _id: 'p1', title: 'Laptop' }, quantity: 1 }],
    };

    axios.get.mockResolvedValue({ data: { data: mockCartData } });
    axios.post.mockResolvedValue({
      data: { data: { summary: { total: 1000, finalAmount: 1000 } } },
    });
    ...
    await waitFor(() => {
      expect(axios.put).toHaveBeenCalledWith('/cart', { productId: 'p1', quantity: 2 });
    });
  });
  ...
  it('Given a product in the cart, When the delete button is clicked, Then the item disappears (Product removed from the cart).', async () => {
    const mockCartData = {
      items: [{ _id: 'item1', productId: { _id: 'p1', title: 'Bad Product' }, quantity: 1 }],
    };
