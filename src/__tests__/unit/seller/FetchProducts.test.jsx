...
describe('FetchProducts - React Query Cache Test', () => {
  ...
  test('Given un composant fetch produits, When la page revisitée, Then les données viennent du cache', async () => {
    ...
    const { result: firstResult, unmount: firstUnmount } = renderHook(() => useSellerProducts(), {
      wrapper,
    });
    ...
    const productsCalls = mockAxios.get.mock.calls.filter((call) => call[0] === '/products').length;
    ...
    const { result: secondResult } = renderHook(() => useSellerProducts(), { wrapper });
